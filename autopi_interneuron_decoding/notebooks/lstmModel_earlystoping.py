...
def get_test_training_datas_oneSession(sSes, ses, cells,interName, ctype= 'fs', sigma_ifr= 5, maxDistance= 18, rotationType="none", shuffle=False):
...
def Get_shuffle_values_one_session(sSes, ses, cells,interName, iteration= 100): 
    results = []
...
def process_session_intervals_withShuffles(sSes, ses, interNames, cells, iteration=100):
...
#############################################
## functions to work with the early stoping and shuffling together 
#############################################

from torch.optim.lr_scheduler import ReduceLROnPlateau

def training_loop_with_early_stopping(n_epochs,
                                      optimizer,
                                      scheduler,
                                      model,
                                      loss_fn,
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for imgs, sin_labels, cos_labels, time in train_data_loader:
            imgs = imgs.to(device)
            sin_labels = sin_labels.to(device).squeeze()
            cos_labels = cos_labels.to(device).squeeze()
        ...
        if verbose:
            print(f"{datetime.datetime.now()} Epoch: {epoch}/{n_epochs}, "
                  f"Training loss: {loss_train/len(train_data_loader):.6f}, "
                  f"Testing loss: {testLoss:.6f}")
...
def run_evalute_model_withEarlystoping(sSes, ses, interName, cells, ctype='fs', sigma_ifr=5, patience=10, factor=0.5, verbose=True): 
    
    '''
    patience: Controls how many epochs to wait for an improvement before adjusting the learning rate or stopping training.
    factor: Multiplier for reducing the learning rate (default: 0.5)
    verbose: printing the output of the training loop including the training and test loss
    ...
    if FS_count >= 1:
        ...
        train_dataset, test_dataset, train_loader, test_loader, _, config = get_test_training_datas_oneSession(
            sSes, ses, cells, interName, ctype=ctype, sigma_ifr=sigma_ifr, maxDistance=18, rotationType="none", shuffle= False)
        ...
        df, best_model_state = training_loop_with_early_stopping(
...
def run_and_evaluate_model_earlystoping_shuffle(sSes, ses, cells,interName, ctype='fs', sigma_ifr=5, patience=20, factor=0.6, shuffle=True, verbose=True):
    
    # Prepare datasets and dataloaders
        train_dataset, test_dataset, train_data_loader, test_data_loader, myDict, config = get_test_training_datas_oneSession(
            sSes, ses, cells, interName, ctype=ctype, sigma_ifr=sigma_ifr, maxDistance=18, rotationType="none", shuffle=shuffle
        )
        
        # Set device and initialize model
        device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
        model = LSTM(
            config["n_cells"], config["hidden_size"], config["num_layers"], 
            config["num_outputs"], config["seq_length"], device=device
        ).to(device)
        ...
        for split, labels, outputs in zip(['train', 'test'], [labels_train, labels_test], [outputs_train, outputs_test]):
            angles[f'Angle_{split}_shuffled'] = np.arctan2(labels[:, 0], labels[:, 1])
            angles[f'Angle_{split}_pred_shuffled'] = np.arctan2(outputs[:, 0], outputs[:, 1])

        # Calculate correlations
        metrics = {
            'r_train': pearsonr(angles['Angle_train_shuffled'], angles['Angle_train_pred_shuffled'])[0],
            'circr_train': circcorrcoef(angles['Angle_train_shuffled'], angles['Angle_train_pred_shuffled']),
            'r_test': pearsonr(angles['Angle_test_shuffled'], angles['Angle_test_pred_shuffled'])[0],
            'circr_test': circcorrcoef(angles['Angle_test_shuffled'], angles['Angle_test_pred_shuffled']),
            'df': df,
        }
        
        return {**angles, **metrics}
    
def Get_shuffle_values_one_session_earlyStoping(sSes, ses, cells,interName, sigma_ifr=5, patience=20, factor=0.6, shuffle=True, iteration= 500, verbose= False): 
    
    results = []
    for _ in tqdm(range(iteration)):
        angles_and_metrics = run_and_evaluate_model_earlystoping_shuffle(sSes, ses, cells,interName, ctype='fs', sigma_ifr=sigma_ifr, 
                                                                         patience=patience, factor=factor, shuffle=shuffle, verbose=verbose)
...
def process_session_intervals_withShuffles_earlystoping(sSes, ses, interNames, cells, sigma_ifr= 5, patience=20, factor=0.6  ,iteration=100, shuffle=True,  verbose= False):
    # Check if there are more than 1 FS cells in the session
    FS_count = cells[(cells.interneuron) & (cells.mrate_RF1 > 10) & (cells.session == sSes.name)].shape[0]
    ...
    if FS_count >= 1 :
        for interName in interNames:
            # Define the file path
            filename = f"{sSes.name}_{interName}_shuffled_values_earlystopping.pkl"
            file_path = os.path.join(save_directory, filename)
            ...
            print(sSes.name)
            #df_sSes = Get_shuffle_values_one_session(sSes, ses,cells, interName, iteration=iteration)
            df_sSes = Get_shuffle_values_one_session_earlyStoping (sSes, ses, cells,interName, sigma_ifr, patience, factor , shuffle, iteration, verbose)
            df_sSes.to_pickle(file_path)
            print(f"Saved data for {filename} at {file_path}")
            del df_sSes
            #gc.collect()
            
def process_session_wrapper_earlystoping(session_pair, interNames, cells, sigma_ifr, patience, factor, iteration, shuffle, verbose):
    
    sSes, ses = session_pair  # Unpack the session pair
    
    process_session_intervals_withShuffles_earlystoping(sSes, ses, interNames, cells, sigma_ifr=sigma_ifr, patience=patience, factor=factor, 
        iteration=iteration, shuffle=shuffle, verbose=verbose
    )
